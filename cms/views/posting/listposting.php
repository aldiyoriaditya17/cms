...
?>
<input type="hidden" id="url-posting-data" value="<?= Url::to(['posting/list']) ?>"/>
<input type="hidden" id="url-filter-data" value="<?= Url::to(['posting/filter']) ?>"/>
...
$script = <<< JS
    ...
    $(document).on('click', '#btn-filter-tag', function (e) {
           ...
           $.ajax({
                ...
                type: "POST",
                data:{"tag":tag,"start_date":start_date,"end_date":end_date,"$param":"$token"},
          ...
          //  let tags=$('#temptag').val();
          //  $('#temptag').val(tags+"#"+tag);
          //  $('#tags-input').val("");
        });
